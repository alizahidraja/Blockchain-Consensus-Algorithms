...
func InsertBlock(Transaction, Reward string, chainHead * Block) * Block {
	if chainHead == nil {
		return &Block {
			Transaction, Reward, nil, DeriveHash(Transaction + "+" + Reward), [32] byte {}
		}
	}
	return &Block {
		Transaction, Reward, chainHead, DeriveHash(Transaction + "+" + Reward), DeriveHash(chainHead.Transaction + "+" + chainHead.Reward)
	}
	...
	if len(os.Args) == 2 { //Satoshi
		...
		for {
			...
			if i == 1 { //Transaction
				...
				gobEncoder: = gob.NewEncoder(conn)
				err = gobEncoder.Encode(Transaction {
					0, amount, j, miner, tranfee, people[miner].Donation
				})
		...
		person: = & Person {
			os.Args[1], 100, os.Args[2], os.Args[3], os.Args[4], os.Args[5], 0
		}
		...
		for {
			...
			if i == 1 { //Transaction
				...
				gobEncoder: = gob.NewEncoder(conn)
				err = gobEncoder.Encode(Transaction {
					ind, amount, j, miner, tranfee, people[miner].Donation
				})
